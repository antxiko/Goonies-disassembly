...
def main():
    ...
    for ln in open(entries_path, encoding="utf-8"):
        ...
        if ln.lower().startswith("!skip"):
            ...
            continue
        if ln.lower().startswith("!tabla"):
            # !tabla 0x409F 9  -> en 0x409F hay una tabla de 9 punteros de 16
            # bits: los 18 bytes son DATOS y cada puntero es un punto de
            # entrada. Es lo que hace falta cuando el despacho es un
            # `pop hl / add a,a / ... / jp (hl)`: el trazado no puede seguirlo
            # y las nueve tablas de este cartucho quedarian sin trazar.
            _, a, n = ln.split()[:3]
            tablas.append((int(a, 0), int(n, 0)))
            continue
    ...
    if os.path.exists(ncpath):
        for ln in open(ncpath, encoding="utf-8"):
            ln = ln.split("#")[0].strip()
            if ln:
                a, b = ln.split()[:2]
    ...
    # Las tablas de punteros: sus bytes son datos, y cada puntero es entrada.
    for a, n in tablas:
        nocode.append((a, a + 2 * n))
        for k in range(n):
            o = a - org + 2 * k
            entries.append(data[o] | data[o + 1] << 8)
    if tablas:
        print(f"tablas de punteros declaradas: {len(tablas)}, "
              f"{sum(n for _, n in tablas)} destinos")
    ...
    r = t.report()
    with open(outprefix + ".trace.json", "w", encoding="utf-8") as f:
